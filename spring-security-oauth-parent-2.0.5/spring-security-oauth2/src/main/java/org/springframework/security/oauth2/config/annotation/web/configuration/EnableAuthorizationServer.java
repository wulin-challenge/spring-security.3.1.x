 ...
 * 
 * <p> 用于在当前应用程序上下文中启用授权服务器（即AuthorizationEndpoint和TokenEndpoint）的便捷注释，
 * 该上下文必须是DispatcherServlet上下文。 可以使用AuthorizationServerConfigurer类型
 * 的@Bean定制服务器的许多功能（例如，通过扩展AuthorizationServerConfigurerAdapter）。
 * 用户负责使用正常的Spring Security功能（@EnableWebSecurity等）保护授权端点（/oauth/authorize），
 * 但是令牌端点（/oauth/token）将使用客户端凭据上的HTTP Basic身份验证自动得到保护。 必须通过通过一个或多个
 * AuthorizationServerConfigurers提供ClientDetailsService来注册客户端。
 * 
 * @author Dave Syer
 * 
 */
