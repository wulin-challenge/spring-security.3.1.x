 ...
 * 
 * <p> 必须在任何身份验证处理机制之前执行此过滤器。身份验证处理机制（例如BASIC，CAS处理过滤器等）希望SecurityContextHolder
 * 在执行时包含有效的SecurityContext。
 * 
 * <p>
 ...
 * 
 * <p> 这实质上是对旧HttpSessionContextIntegrationFilter的重构，以将存储问题委派给单独的策略，
 ...
 * 
 * <p> forceEagerSessionCreation属性可用于确保会话在过滤器链执行之前始终可用（默认值为false，因为这会占用大量资源，因此不建议使用）。
 *
 * @author Luke Taylor
 * @since 3.0
 */
public class SecurityContextPersistenceFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        try {
            ...
            // 至关重要的是删除SecurityContextHolder内容-在执行其他操作之前。
