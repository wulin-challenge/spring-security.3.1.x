 ...
 * 
 * <p> AbstractSecurityInterceptor将确保安全拦截器的正确启动配置。它还将实现对安全对象调用的正确处理，即：
 * 
 * <ol>
 * <li>Obtain the {@link Authentication} object from the {@link SecurityContextHolder}.</li>
 * <li>
 * <li>从SecurityContextHolder获取身份验证对象。
 * <li>
 ...
 * <li>
 * <li> 如果org.springframework.security.core.Authentication.isAuthenticated（）
 * 返回false，或者alwaysReauthenticate为true，则进行身份验证针对已配置的AuthenticationManager的请求。
 * 通过身份验证后，将SecurityContextHolder上的Authentication对象替换为返回的值。
 * <li>
 * <li>Authorize the request against the configured {@link AccessDecisionManager}.</li>
 * <li>
 * <li> 针对已配置的AccessDecisionManager授权请求。
 * <li>
 * <li>Perform any run-as replacement via the configured {@link RunAsManager}.</li>
 * <li>
 * <li> 通过配置的RunAsManager执行任何运行方式替换。
 * <li>
 ...
 * <li>
 * <li> 将控制传递回具体的子类，该子类实际上将继续执行该对象。返回InterceptorStatusToken，以便子类完成对象的执行后，
 * 其finally子句可以确保调用AbstractSecurityInterceptor并使用finallyInvocation（InterceptorStatusToken）
 * 进行正确整理。
 * <li>
 ...
 * <li>
 * <li> 具体的子类将通过afterInvocation（InterceptorStatusToken，Object）方法重新调用AbstractSecurityInterceptor。
 * <li>
 ...
 * <li>
 * <li> 如果RunAsManager替换了Authentication对象，则将SecurityContextHolder返回到调用AuthenticationManager之后存在的对象
 * <li>
 ...
 * the caller.</li>
 * <li>
 * <li> 如果定义了AfterInvocationManager，则调用调用管理器并允许它替换要返回给调用方的对象。
 * </ol>
 * </li>
 ...
 * <li>
 * <li> 对于公共调用（安全对象调用没有ConfigAttributes）：a。如上所述，将向具体子类返回InterceptorStatusToken，该对象随后在执行安全对象后重新呈现给
 * AbstractSecurityInterceptor 。当调用其afterInvocation（InterceptorStatusToken，Object）时，
 * AbstractSecurityInterceptor将不再采取进一步行动。
 * </ol>
 * </li>
 ...
 * <li>
 * <li> Control再次返回到具体的子类，以及应返回给调用者的Object。然后，子类会将结果或异常返回给原始调用者。
 * </ol>
 *
 * @author Ben Alex
 * @author Rob Winch
